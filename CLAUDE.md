...
- **Resuming:** find the most recent **Active** arc in `story/campaign-log.md`, read its `arc.md` (summary), its latest job file, `story/open-threads.md`, and `characters/lee-cadar.md`, before picking play back up — don't ask the player to re-summarize what's already recorded here.
  - Do this at the start of **any** session where it hasn't been done yet in the current conversation — including right after a `/compact`, and even if the first message looks like small talk ("hello?") rather than an explicit "let's continue." A carried-over conversation summary only reflects that conversation's own history; it cannot see campaign progress made in other sessions, so treat it as a rough pointer, not ground truth. The repo files (and `git log`, if the summary's account of recent commits seems thin) are the actual source of truth — verify against them before acting on a summary.
  - The player can also trigger this explicitly with `/resume` (`.claude/commands/resume.md`) — a `git pull` + full sync-and-recap pass. Good habit at the start of every session.
