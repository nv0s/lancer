# Voss "Doc" Amara — Support

**License Level:** LL1
**Background:** Doctor
**Grit:** 1

Late 40s, unhurried, missing two fingers on her left hand and never explains how. Was a trauma surgeon on a core world before a manna shortfall gutted her hospital's funding and she went where the money still moved: the frontier. Doesn't flinch, doesn't preach, but keeps a running tally of who owes her a favor for patching them up. The closest thing the crew has to a conscience — she's seen what happens when contracts go bad and mercs stop asking who's paying them.

Dry, economical with words. Calls Lee "sniper boy" without a hint of affection or malice — it's just what he is to her until proven otherwise.

## Triggers (+2 each)
- **Patch** — medicate, bandage, suture, cauterize, neutralize poison, resuscitate, diagnose disease
- **Read a Situation** — discern motivations, sense who's about to do something rash, get a gut feeling, sense a lie

## Pilot Combat Stats
SIZE 1/2 · HP 6 · EVASION 10 · E-DEFENSE 10 · SPEED 4

## Talents
- **Grease Monkey** (Rank I) — *Unsanctioned Capacity Upgrade*: while resting, spend 2 Repairs to replenish 1 use of all LIMITED weapons/systems.

## Pilot Gear
- **Light Hardsuit** — +3 HP, Armor 0, Evasion 10, E-Defense 10, Speed 4
- **Signature Weapon, Medium (A/C)** — a surgeon's blade, reflavored; Threat 1, 2 kinetic damage
- **Patch** (Limited 1, Full Action) — apply to self or adjacent pilot, restores half max HP
- **Corrective** (Limited 1, Full Action) — apply to a DOWN AND OUT pilot, brings them back to consciousness at 1 HP

---

# Mech: *(unnamed)*

**Frame:** IPS-N Lancaster — a mil-spec variant of an IPS-N civilian transport/maintenance chassis, streamlined for frontline support. Lancaster pilots often serve as sappers and engineers; the frame carries allies into the fight and repairs them mid-combat.

## Core Stats
Size 2 · Armor 1 · Save Target 10 · Sensors 8

| Stat | Value | Source |
|---|---|---|
| HP | 7 | 6 base + 1 (Grit) |
| Repair Cap | 10 | base |
| Evasion | 9 | 8 base + 1 (Agility 1) |
| Speed | 6 | base |
| E-Defense | 10 | 8 base + 2 (Systems 2) |
| Tech Attack | +3 | base +1 + 2 (Systems 2) |
| Heat Cap | 6 | base |
| SP | 9 | 8 base + 1 (Grit) |
| Save Target | 11 | 10 base + 1 (Grit) |
| Attack Rolls | +1 | Grit bonus applies to all mech attack rolls |

## Mech Skills
Hull 0 · **Agility 1** · **Systems 2** · Engineering 0

## Traits (from Lancaster frame)
- **Insulated** — immunity to burn damage
- **Combat Repair** — in combat, can spend 4 Repairs to repair a destroyed mech as a full action, returning it to 1 Structure, 1 Stress, 1 HP
- **Redundant Systems** — other characters adjacent to the Lancaster can spend its Repairs as their own

## Core System
**Latch Drone** (*Integrated Mount*) — a companion drone deployed from the chassis. Can't make normal attacks; instead, targets an allied mech within Range 8 and LOS with a ranged attack against Evasion 8 — on a hit, either pilot may spend 1 Repair to restore half the target's HP.

**Supercharger** (*Active, 1CP, Quick Action*) — the Latch Drone clamps onto an allied mech within range; for the rest of the scene that ally takes 1 heat at the start of each of Doc's turns, but gains +1 accuracy on all attacks/checks/saves and immunity to Impaired, Jammed, Slowed, Shredded, and Immobilized from other sources. Ends if either character is Stunned. While active, the Latch Drone can't be used for its normal repair function.

## Mounts
- **Main/Aux:** Precision Rifle — Main Rifle, ACCURATE, R10, 1d6+2 — purchased (3,500c) for reach and reliability without pulling Doc away from her support role; mid-range, hard to miss with.

## Systems (3/8 SP)
- **Cable Winch System** (1 SP, Quick Action) — attach cables to an adjacent character to tow, drag, or restrain them; also usable to climb surfaces or interact with the environment
- **Restock Drone** (2 SP, Limited 2, Unique, Quick Action) — deploy a drone that lets adjacent allies clear 1d6 heat and one condition, or reload one LOADING weapon
